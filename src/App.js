...
const data = [
  {
    name: "Shruthi",
    img: "https://webstockreview.net/images/clipart-boy-phone-11.png"
  },
  {
    name: "Radhika",
    img:
      "https://clipground.com/images/hindu-woman-clipart-5.png"
  },
  {
    name: "Sridhar",
    img:
      "https://images.onlinelabels.com/images/clip-art/Juhele/Business%20man%20using%20a%20phone-284999.png"
  }
];
const movies = [
  {
...
function Msg({ name, img }) {
  return (
    <div className="msg-spec">
       <h1> Hi {name}!</h1> &nbsp;
        <img src={img} width="50" height="50" alt="PersonImage" />
    </div>
  );
}
function Movies({ name, poster, summary, rating, cast }) {
  return (
    <div className="movie-data">
      ...
      <p className="movie-cast"> <span className="title">Cast:</span>{cast.toString()} </p>
      <Counter />
    </div>
  );
}
...
function App() {
  return (
    <div>
       <h1 className="heading">Hello Messages</h1>
       <div className="msg-arrangement">
      {data.map(({ name, img }) => (
        <Msg name={name} img={img} />
      ))}
      </div>
      <h1 className="heading">Movies Details</h1>
      <div className="movies-arrangement">
      {movies.map(({ name, poster, summary, rating, cast }) => (
        <Movies
          name={name}
          poster={poster}
          summary={summary}
